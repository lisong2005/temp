...
package com.witon.ehealth.util.qr;
...
public class PRUtil {
    ...
    */
     ...
     * @param destImagePath
     */
    public static void encodePR(String content, String charset, int width, int height,
                                String srcImagePath, String destImagePath) {
        try {
            ImageIO.write(genBarcode(content, charset, width, height, srcImagePath), "jpg",
     ...
     * @return
     */
    public static String decodePR(String imgPath, String charset) {
        BufferedImage image = null;
        Result result = null;
        try {
            image = ImageIO.read(new File(imgPath));
            if (image == null) {
                logger.warn("the decode image may be not exists.");
            }
    ...
    public static void encodeBar(String contents, int width, int height, String imgPath) {
        ...
        try {
            BitMatrix bitMatrix = new MultiFormatWriter().encode(contents, BarcodeFormat.EAN_13,
     ...
     * @throws IOException
     */
    private static BufferedImage genBarcode(String content, String charset, int width, int height,
                                            String srcImagePath) throws WriterException,
                                                                 IOException {
        // 读取源图像
        ...
        for (int y = 0; y < matrix.getHeight(); y++) {
            for (int x = 0; x < matrix.getWidth(); x++) {
                ...
                if (x > halfW - IMAGE_HALF_WIDTH && x < halfW + IMAGE_HALF_WIDTH
                    ...
                    pixels[y * width
                           + x] = srcPixels[x - halfW + IMAGE_HALF_WIDTH][y - halfH
                                                                          + IMAGE_HALF_WIDTH];
                }
                // 在图片四周形成边框
